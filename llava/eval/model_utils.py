...
def load_video(vis_path, n_clips=1, num_frm=100):
    ...
    # target_h, target_w = 336, 336 # 672, 672
    # size = (target_w, target_h)
    # sizes = (size,) * total_num_frm
    ...
    #     (total_num_frm, img_array.shape[-3], img_array.shape[-2], img_array.shape[-1]))
    #####-------- pre resize end------------------####    
...
def load_video_key_frames(vis_path, num_frm, key_idx):
    """
    Load video frames from a video file.
    ...
    n_clips = 1
    # Get indices of frames to extract
    select_idx = key_idx[:num_frm] if num_frm < len(key_idx) else key_idx
    sorted(select_idx)
    # Calculate total number of frames to extract
    total_num_frm = min(total_frame_num, len(select_idx))
    # Get indices of frames to extract
    # Extract frames as numpy array
    img_array = vr.get_batch(select_idx).asnumpy()
...
def load_video_uniform_key_frames(vis_path, num_frm, key_idx):
    """
    Load video frames from a video file.
    ...
    key_frame_idx = key_idx[:(total_num_frm//2)]
    sorted(key_frame_idx)
    
    uni_frame_idx = get_seq_frames(total_frame_num, total_num_frm//2)

    frame_idx = set(key_frame_idx + uni_frame_idx)

    for i in range(total_num_frm//2, len(key_idx)):
        if len(frame_idx) == total_num_frm:
            break
        frame_idx.add(key_idx[i])

    select_idx = list(frame_idx)
...
def initialize_model(model_name, projection_path=None):
    ...
    image_processor = CLIPImageProcessor.from_pretrained(model.config.mm_vision_tower, torch_dtype=torch.float16, cache_dir='./cache_dir')
